...
class PatientDestroyTestCase(TestSetUp):
    def setUp(self):
        super().setUp()
        self.url = lambda id: reverse("patients-detail", kwargs={"id": id})
    ...
    def test_destroy_non_existing_patient(self):
        self.client.force_authenticate(user=self.doctor)
    ...
    def test_destroy_patient_with_owner_user(self):
        self.client.force_authenticate(user=self.patient_with_address_and_primary_doctor)
        response = self.client.delete(self.url(self.patient_with_address_and_primary_doctor.id))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_destroy_patient_with_non_authenticated_user(self):
        response = self.client.delete(self.url(self.patient_with_address_and_primary_doctor.id))
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
